...
static auto scan_tokens(const std::string &source)
    -> std::vector<lox::syntax::Token> {
  ...
  while (true) {
    ...
    if (!token_result) {
      throw std::runtime_error(
          std::format("Lexing error: {}", token_result.error()));
    }
...
static auto test_parse_literal_expression() -> bool {
  ...
  const auto *expr_stmt =
      dynamic_cast<const lox::ast::ExpressionStatement *>(statements[0].get());
  if (!expr_stmt) {
    std::print("Expected expression statement\n");
    return false;
  }

  const auto *literal = dynamic_cast<const lox::ast::LiteralExpression *>(
...
static auto test_parse_binary_expression() -> bool {
  ...
  const auto *binary = dynamic_cast<const lox::ast::BinaryExpression *>(
      expr_stmt->expression.get());
  if (!binary) {
...
static auto test_parse_unary_expression() -> bool {
  ...
  const auto *unary = dynamic_cast<const lox::ast::UnaryExpression *>(
      expr_stmt->expression.get());
  if (!unary) {
...
static auto test_parse_grouping_expression() -> bool {
  ...
  const auto *grouping =
      dynamic_cast<const lox::ast::GroupingExpression *>(binary->left.get());
  if (!grouping) {
...
static auto test_parse_variable_declaration() -> bool {
  ...
  const auto *var_stmt =
      dynamic_cast<const lox::ast::VarStatement *>(statements[0].get());
  if (!var_stmt) {
...
static auto test_parse_assignment_expression() -> bool {
  ...
  const auto *assignment = dynamic_cast<const lox::ast::AssignmentExpression *>(
      expr_stmt->expression.get());
  if (!assignment) {
...
static auto test_parse_print_statement() -> bool {
  ...
  const auto *print_stmt =
      dynamic_cast<const lox::ast::PrintStatement *>(statements[0].get());
  if (!print_stmt) {
...
static auto test_parse_if_statement() -> bool {
  ...
  const auto *if_stmt =
      dynamic_cast<const lox::ast::IfStatement *>(statements[0].get());
  if (!if_stmt) {
...
static auto test_parse_while_statement() -> bool {
  ...
  const auto *while_stmt =
      dynamic_cast<const lox::ast::WhileStatement *>(statements[0].get());
  if (!while_stmt) {
...
static auto test_parse_function_declaration() -> bool {
  ...
  const auto *fun_stmt =
      dynamic_cast<const lox::ast::FunctionDeclarationStatement *>(
          statements[0].get());
  if (!fun_stmt) {
...
static auto test_parse_block_statement() -> bool {
  ...
  const auto *block_stmt =
      dynamic_cast<const lox::ast::BlockStatement *>(statements[0].get());
  if (!block_stmt) {
...
static auto test_parse_logical_expression() -> bool {
  ...
  const auto *logical = dynamic_cast<const lox::ast::LogicalExpression *>(
      expr_stmt->expression.get());
  if (!logical) {
