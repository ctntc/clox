...
private:
  ...
  auto current_token() const noexcept -> const syntax::Token &;
  auto previous_token() const noexcept -> const syntax::Token &;
  auto peek_token() const noexcept -> const syntax::Token &;
  auto is_at_end() const noexcept -> bool;
  auto advance() noexcept -> const syntax::Token &;
  auto check(syntax::TokenKind kind) const noexcept -> bool;
  auto match(std::initializer_list<syntax::TokenKind> kinds) noexcept -> bool;
  auto consume(syntax::TokenKind kind, const std::string &message) noexcept
  ...
  auto parse_literal() noexcept -> std::expected<ExprPtr, std::string>;
  auto parse_variable() noexcept -> std::expected<ExprPtr, std::string>;

  auto finish_call(ExprPtr callee) noexcept
      -> std::expected<ExprPtr, std::string>;

  auto get_rule(syntax::TokenKind kind) const noexcept -> const ParseRule &;
  auto get_precedence(syntax::TokenKind kind) const noexcept -> Precedence;
