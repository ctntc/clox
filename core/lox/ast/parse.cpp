...
auto Parser::check(syntax::TokenKind kind) const noexcept -> bool {
  if (is_at_end())
    return false;
  return current_token().kind == kind;
}

auto Parser::match(std::initializer_list<syntax::TokenKind> kinds) noexcept
    -> bool {
...
auto Parser::consume(syntax::TokenKind kind,
...
auto Parser::parse_logical(ExprPtr left) noexcept
  ...
  Precedence precedence = Precedence::logical_and;
...
auto Parser::parse_assignment(ExprPtr left) noexcept
  ...
  if (auto var_expr = dynamic_cast<VariableExpression *>(left.get())) {
...
auto Parser::parse_literal() noexcept -> std::expected<ExprPtr, std::string> {
...
auto Parser::parse_variable() noexcept -> std::expected<ExprPtr, std::string> {
  const auto &token = previous_token();
  spdlog::trace("Parser: parse_variable with name: {}", token.lexeme);
  return std::make_unique<VariableExpression>(token);
}

auto Parser::get_rule(syntax::TokenKind kind) const noexcept
    -> const ParseRule & {
  static const ParseRule empty_rule{nullptr, nullptr, Precedence::none};
...
auto Parser::get_precedence(syntax::TokenKind kind) const noexcept
    -> Precedence {
...
auto Parser::synchronize() noexcept -> void {
  ...
  while (!is_at_end()) {
    ...
    if (current_token().kind == syntax::TokenKind::keyword) {
      ...
      if (lexeme == "class" || lexeme == "fun" || lexeme == "var" ||
...
void Parser::initialize_parse_rules() noexcept {
  ...
  // Add specific rules for different operator precedences
  ...
  rules_[syntax::TokenKind::simple_operator] = ParseRule{
      ...
      [this](ExprPtr left) -> std::expected<ExprPtr, std::string> {
        const auto &token = previous_token();
        if (token.lexeme == "=") {
