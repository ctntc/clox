...
namespace lox::syntax {
	...
	Scanner::Scanner(const std::string_view source) noexcept
		...
		spdlog::debug("Scanner: Initialized with source of length {}",
			source_.length());
	}
	...
	auto Scanner::is_at_end() const noexcept -> bool {
		return cursor_.pos >= source_.length();
	}

	auto Scanner::current_char() const noexcept -> char {
		return is_at_end() ? '\0' : source_[cursor_.pos];
	}

	auto Scanner::peek_char(const size_t offset) const noexcept -> char {
		const size_t pos =
			static_cast<size_t>(cursor_.pos) + static_cast<size_t>(offset);
	...
	auto Scanner::scan_identifier() noexcept -> std::expected<Token, std::string> {
		...
		if (const auto it = keywords.find(lexeme); it != keywords.end()) {
			spdlog::trace("Scanner: Scanned keyword '{}' at span [{}, {})", lexeme,
				span.start, span.end);
			return Token::make(it->second, lexeme, span);
		}

		spdlog::trace("Scanner: Scanned identifier '{}' at span [{}, {})", lexeme,
			span.start, span.end);
	...
	auto Scanner::scan_number() noexcept -> std::expected<Token, std::string> {
		...
		spdlog::trace("Scanner: Scanned number '{}' at span [{}, {})", lexeme,
			span.start, span.end);
	...
	auto Scanner::scan_string() noexcept -> std::expected<Token, std::string> {
		...
		if (is_at_end()) {
			spdlog::error(
				"Scanner: Unterminated string literal starting at position {}", start);
		...
		advance();
		const auto span =
			Span{ .start = static_cast<uint8_t>(start), .end = cursor_.pos };

		spdlog::trace("Scanner: Scanned string '{}' at span [{}, {})", lexeme,
			span.start, span.end);
	...
	auto Scanner::scan_operator() noexcept -> std::expected<Token, std::string> {
		...
		case '/':
			break;
		default:
			spdlog::error("Scanner: Unknown operator '{}' at position {}", first_char,
				start);
			return std::unexpected(
				std::format("Unknown operator '{}' at position {}", first_char, start));
		}

		const auto lexeme = source_.substr(start, length);
		const auto span = Span{ .start = start, .end = cursor_.pos };

		const auto kind_str = (kind == TokenKind::compound_operator)
			? "compound operator"
			: "simple operator";
		spdlog::trace("Scanner: Scanned {} '{}' at span [{}, {})", kind_str, lexeme,
			span.start, span.end);
	...
	auto Scanner::get_next_token() noexcept -> std::expected<Token, std::string> {
		...
		if (current == '(' || current == ')' || current == '{' || current == '}' ||
			...
			spdlog::trace("Scanner: Scanned punctuation '{}' at span [{}, {})", lexeme,
				span.start, span.end);
		...
		const auto start = cursor_.pos;
		advance();
		spdlog::error("Scanner: Unexpected character '{}' at position {}", current,
			start);
		return std::unexpected(
			std::format("Unexpected character '{}' at position {}", current, start));
	}
